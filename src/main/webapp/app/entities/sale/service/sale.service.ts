...
type RestOf<T extends ISale | NewSale> = Omit<T, 'createDate' | 'modifiedDate'> & {
...
export class SaleService {
  ...
  protected convertDateFromClient<T extends ISale | NewSale | PartialUpdateSale>(sale: T): RestOf<T> {
    return {
      ...sale,
      createDate: sale.createDate?.format(DATE_FORMAT) ?? null,
      modifiedDate: sale.modifiedDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSale: RestSale): ISale {
    return {
      ...restSale,
      createDate: restSale.createDate ? dayjs(restSale.createDate) : undefined,
      modifiedDate: restSale.modifiedDate ? dayjs(restSale.modifiedDate) : undefined,
    };
  }
