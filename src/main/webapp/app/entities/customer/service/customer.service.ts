...
type RestOf<T extends ICustomer | NewCustomer> = Omit<T, 'createDate' | 'modifiedDate'> & {
...
export class CustomerService {
  ...
  protected convertDateFromClient<T extends ICustomer | NewCustomer | PartialUpdateCustomer>(customer: T): RestOf<T> {
    return {
      ...customer,
      createDate: customer.createDate?.format(DATE_FORMAT) ?? null,
      modifiedDate: customer.modifiedDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCustomer: RestCustomer): ICustomer {
    return {
      ...restCustomer,
      createDate: restCustomer.createDate ? dayjs(restCustomer.createDate) : undefined,
